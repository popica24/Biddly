            ...
            using (var scope = _serviceScopeFactory.CreateScope())
            {
                ...
                foreach (var bidModel in runningBids.ToList())
                {
                    if (bidModel != null && bidModel.WonAt <= now)
                    {
                        ...
                        var finishBidCommand = new FinishBidCommand(bidModel);
                        await sender.Send(finishBidCommand, stoppingToken);

                        await _biddingHub.Clients.All.AnnounceWinner(bidModel.BidId);
                    }
                }
            }
